...
def show_editable_profile(user):
    ...
    other_users = [u for u in data_manager.users if u.user_id != user.user_id]
    
    if other_users:
        with st.form("send_message_form"):
            recipient_options = [(u.user_id, f"{u.name} ({u.role})") for u in other_users]
            ...
            if send_submitted:
                if message_content:
                    ...
                    # Send message
                    data_manager.send_message(user.user_id, recipient_id, message_content)
...
def show_read_only_profile(profile_user, current_user):
    ...
    with st.form("send_direct_message_form"):
        ...
        if send_submitted:
            if message_content:
                # Send message
                data_manager.send_message(current_user.user_id, profile_user.user_id, message_content)
