...
def show_dashboard(start_date, end_date):
    ...
    bookings = hotel_data.get_bookings(start_date, end_date)
    ...
    # Create charts
    tab1, tab2, tab3 = st.tabs(["Occupancy", "Revenue", "Bookings"])
    ...
    with tab2:
        if not df_revenue.empty:
            fig = px.line(df_revenue, x='date', y=['total_revenue', 'room_revenue', 'food_beverage_revenue', 'other_revenue'], 
